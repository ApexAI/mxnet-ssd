...
def shuffleUnit(residual, in_channels, out_channels, combine_type, groups=3, grouped_conv=True,name=""):
    if combine_type == 'add':
        DWConv_stride = 1
    ...
    data = mx.sym.Convolution(data=residual, num_filter=bottleneck_channels,
                              kernel=(1, 1), stride=(1, 1), num_group=first_groups,name=name+"_conv1")
    data = mx.sym.BatchNorm(data=data)
    data = mx.sym.Activation(data=data, act_type='relu',name=name+"_act1")

    data = channel_shuffle(data, groups)

    data = mx.sym.Convolution(data=data, num_filter=bottleneck_channels, kernel=(3, 3),
                              pad=(1, 1), stride=(DWConv_stride, DWConv_stride), num_group=groups,name=name+"_conv2")
    data = mx.sym.BatchNorm(data=data)

    data = mx.sym.Convolution(data=data, num_filter=out_channels,
                              kernel=(1, 1), stride=(1, 1), num_group=groups,name=name+"_conv3")
    data = mx.sym.BatchNorm(data=data)

    if combine_type == 'concat':
        residual = mx.sym.Pooling(data=residual, kernel=(3, 3), pool_type='avg',
                                  stride=(2, 2), pad=(1, 1),name=name+"_pooling")
...
def make_stage(data, stage, name="", groups=3):
    stage_repeats = [3, 7, 3]
...
def get_symbol(num_classes, **kwargs):
    ...
    data = make_stage(data, 2, "stage1")

    data = make_stage(data, 3, "stage2")

    data = make_stage(data, 4, "stage3")

    data = mx.sym.Pooling(data=data, kernel=(1, 1), global_pool=True, pool_type='avg',name="final_pool")
