...
parser.add_argument('--train-rec', dest='train_rec', help='train file to use',
                    default="../data/nexar/nexar_train_noisy.rec", type=str)

parser.add_argument('--val-rec', dest='val_rec', help='val file to use',
...
parser.add_argument('--num-class', dest='num_class', help='batch size',
                    default=4, type=int)
...
if __name__ == '__main__':
    ...
    if(pargs.resume>-1):
        ...
        if(pargs.net=="resnet18"):
            sym=resnet.get_symbol(pargs.num_class, 18, "3,{},{}".format(pargs.shape,pargs.shape))
        elif (pargs.net=="mobilenet050"):
            sym=mobilenet.get_symbol(pargs.num_class, alpha=0.50)
        elif (pargs.net=="densenet30"):
            sym=densenet.DenseNet(units=[9,9,9], num_stage=3, growth_rate=12, num_class=pargs.num_class, data_type="nexar",
                              bottle_neck=False)
        args = None
        auxs = None
        epoch=0
    ...
    train_iter = mx.io.ImageRecordIter(
      ...
      batch_size=pargs.batch_size, data_shape=(3,pargs.shape,pargs.shape), shuffle=True, rand_crop=True, rand_mirror=True, max_rotate_angle=20, max_shear_ratio=0.1,
    ...
    mod = mx.mod.Module(symbol=sym, context=(mx.gpu()), fixed_param_names=fixed_param_names)
